...
int tcpListen(struct sockaddr_in *addr, uint16_t port) {
    ...
    if (fd != -1) {
        int opt = 1;
        if (-1 != assert("setsockopt", setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, (char *)&opt, sizeof opt)) && -1 != assert("bind", bind(fd, setPort(addr, port), sizeof *addr)) && -1 != assert("listen", listen(fd, 3))) {
