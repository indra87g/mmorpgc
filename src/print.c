...
static void printDemux(OBJECT *obj1, OBJECT *obj2) {
    if (currentSocket == STDOUT_FILENO || player == nobody) {
        ...
        for (i = 0; (client = clientGet(i)) != NULL; i++) {
            if (client->fd != -1 && client->obj->location == player->location && filter(client->obj, obj1, obj2)) {
...
static void printObserve(OBJECT *obj1, OBJECT *obj2, const char *sense, const char *format, va_list ap) {
    outbufClear();
...
void printAny(OBJECT *obj1, OBJECT *obj2, const char *sense, const char *format, ...) {
